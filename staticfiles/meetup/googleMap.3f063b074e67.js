...
function initMap() {
  ...
  if (lastSeg == 'create') {
	...
	document.getElementById('id_location').value = JSON.stringify(sjsu);
  }
  ...
  // If current webpage is either create or update event, listen for clicks on map, updating marker and
  // location field to the geocoordinates of clicked point
  if (lastSeg != 'event') {
...
// Add description boxes when marker is clicked in explore events page
...
// Utility for location search bar, converting address to geocoordinates and updating marker to that location
